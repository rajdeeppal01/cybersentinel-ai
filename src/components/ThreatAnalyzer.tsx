...
import { Play, HelpCircle, Code, ShieldAlert, Cpu, FileText, Sparkles, AlertCircle, Download, CheckCircle } from 'lucide-react';
...
export default function ThreatAnalyzer({ initialLogText }: ThreatAnalyzerProps) {
  ...
  const [completedSteps, setCompletedSteps] = useState<boolean[]>(
    initialLogText ? new Array(analyzeLogLocal(initialLogText).incidentResponsePlaybook.length).fill(false) : []
  );
    ...
    try {
      if (useLocalLLM) {
        try {
          ...
          setNeedsEscalation(result.needsEscalation);
          setCompletedSteps(new Array(result.incidentResponsePlaybook.length).fill(false));
        } catch (e: any) {
          console.warn("WebLLM failed, falling back to local heuristic engine", e);
  ...
  const handleEscalate = async () => {
    ...
    setOnlineError('');
    try {
      const result = await analyzeLogWithGemini(logInput); // Backend handles logic
  ...
  const isAllStepsCompleted = completedSteps.length > 0 && completedSteps.every(Boolean);

  const exportPDF = async () => {
    if (!reportRef.current) return;
    const canvas = await html2canvas(reportRef.current, { scale: 2, backgroundColor: '#03050a' });
    const imgData = canvas.toDataURL('image/png');
    const pdf = new jsPDF('p', 'mm', 'a4');
    const pdfWidth = pdf.internal.pageSize.getWidth();
    const pdfHeight = (canvas.height * pdfWidth) / canvas.width;
    pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, pdfHeight);
    pdf.save('CyberSentinel-Threat-Report.pdf');
  };
    ...
    <div className="cyber-grid-container" style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '20px', padding: '20px' }}>
      ...
      <div className="cyber-panel" style={{ gridColumn: 'span 5', display: 'flex', flexDirection: 'column', gap: '15px' }}>
        ...
        <div style={{ background: 'rgba(0, 240, 255, 0.05)', border: '1px solid var(--neon-cyan)', padding: '12px', borderRadius: '4px' }}>
          ...
          <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)', marginBottom: '10px', lineHeight: '1.4' }}>
            Enable to run the 1.1B parameter Llama model directly in your browser using WebGPU. No data leaves your machine. (Requires 1GB download on first run).
          </p>
        ...
        {loadStatus && (
          <div style={{ fontSize: '0.7rem', color: 'var(--neon-cyan)', background: 'rgba(0,240,255,0.05)', padding: '10px', borderRadius: '4px' }}>
            {loadStatus.text}
          </div>
        )}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
            ...
            {analysisResult && (
                <button 
                  ...
                  onClick={exportPDF}
                >
                    <Download style={{ width: '12px', height: '12px' }} /> Export PDF
                </button>
            )}
        </div>

        <div ref={reportRef} style={{ flex: 1, display: 'flex', flexDirection: 'column', backgroundColor: '#03050a' }}>
            {!analysisResult ? (
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                ...
                <div style={{ border: '1px solid var(--neon-orange)', color: 'var(--neon-orange)', background: 'rgba(255, 159, 0, 0.05)', padding: '10px', borderRadius: '4px', fontSize: '0.75rem', display: 'flex', alignItems: 'flex-start', gap: '10px', marginBottom: '15px' }}>
                    ...
                    <div>
                    <strong style={{ display: 'block', marginBottom: '4px' }}>⚠️ Note: The local on-device AI failed to initialize.</strong>
                    This analysis was dynamically generated by the secure Vercel Python fallback backend instead of your local browser.
                    </div>
                </div>
                )}
                ...
                <div style={{ border: '1px solid var(--neon-orange)', borderRadius: '4px', marginBottom: '20px', overflow: 'hidden' }}>
                    ...
                    The local engine found generic anomalous activity. Escalation to an AI model is recommended.
                    </div>
                    ...
                    <div style={{ padding: '12px', background: 'rgba(255, 159, 0, 0.02)' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            ...
                            >
                            <Sparkles style={{ width: '14px', height: '14px' }} /> Escalate to Vercel Agent Backend
                            </button>
                ...
                <div style={{ border: '1px solid var(--neon-cyan)', borderRadius: '4px', marginBottom: '20px', overflow: 'hidden' }}>
                    ...
                    <Sparkles style={{ width: '14px', height: '14px' }} /> ONLINE MODEL SECOND OPINION (Backend)
                    </div>
                ...
                {/* Diagnostic Subtabs */}
                <div style={{ display: 'flex', borderBottom: '1px solid rgba(0, 240, 255, 0.1)', marginBottom: '15px', paddingTop: '10px' }}>
                <button 
                ...
                {activeSubTab === 'playbook' && (
                    ...
                    <div>
                        ...
                        {analysisResult.incidentResponsePlaybook.map((step, idx) => (
                            <li key={idx} style={{ display: 'flex', gap: '10px', background: '#070b15', padding: '10px', borderRadius: '4px', border: '1px solid rgba(255,255,255,0.03)', alignItems: 'center' }}>
                            <input 
                                type="checkbox" 
                                checked={completedSteps[idx] || false}
                                onChange={() => toggleStep(idx)}
                                style={{ accentColor: 'var(--neon-green)', width: '16px', height: '16px', cursor: 'pointer' }}
                            />
                            <span className="tech-font" style={{ color: completedSteps[idx] ? 'var(--text-muted)' : 'var(--neon-green)', fontWeight: 'bold' }}>0{idx + 1}.</span>
                            <span style={{ textDecoration: completedSteps[idx] ? 'line-through' : 'none', color: completedSteps[idx] ? 'var(--text-muted)' : 'inherit' }}>{step}</span>
                            </li>
                        ))}
                        </ul>
                        
                        {isAllStepsCompleted && (
                            <div style={{ marginTop: '15px', padding: '15px', background: 'rgba(0, 255, 100, 0.1)', border: '1px solid var(--neon-green)', borderRadius: '4px', color: 'var(--neon-green)', display: 'flex', alignItems: 'center', gap: '10px' }}>
                                <CheckCircle style={{ width: '20px', height: '20px' }} />
                                <strong className="tech-font">ALERT RESOLVED: All containment steps completed.</strong>
                            </div>
                        )}
